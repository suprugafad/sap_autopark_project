...
sap.ui.define([
	...
	"by/iba/ZAP_AUTOPARK_11/model/grouper",
	...
	return BaseController.extend("by.iba.ZAP_AUTOPARK_11.controller.Master", {
		...
		onAddCar: function() {
			...
			if (!this.byId("NewCarDialog")) {
				sap.ui.core.Fragment.load({
					id: oView.getId(),
					name: "by.iba.ZAP_AUTOPARK_11.view.fragments.NewCarDialog",
					controller: this
		...
		onAddCarInfo: function() {
			...
			// var sPath = this.getView().byId("newCarDialog").getBindingContext().getPath();
			...
			oODataModel.create("/CarSet", oCar, {
				success: function(oResponse) {
					MessageToast.show("Авто было добавлено");
					this.byId("NewCarDialog").close();
					var oList = this.byId("list");
					oList.getBinding("items").refresh();
				}.bind(this),
				error: function(oError) {
					MessageToast.show("Ошибка добавления");
				}
			});
		},
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("MARK", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("by.iba.ZAP_AUTOPARK_11.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("COSTS", FilterOperator.LE, 100));
						break;
					case "Filter2":
						aFilters.push(new Filter("COSTS", FilterOperator.GT, 100));
						break;
					default:
						break;
				}
		...
		_onMasterMatched: function() {
			this.getOwnerComponent().oListSelector.oWhenListLoadingIsDone.then(
				function(mParams) {
					...
					var sObjectId = mParams.firstListitem.getBindingContext().getProperty("NUM");
