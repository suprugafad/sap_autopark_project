sap.ui.define([
	...
	return BaseObject.extend("by.iba.ZAP_AUTOPARK_11.model.GroupSortState", {
		 ...
		 * @alias by.iba.ZAP_AUTOPARK_11.model.GroupSortState
		 */
		...
		/**
		 * Groups by COSTS, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "COSTS") {
				...
				this._oViewModel.setProperty("/sortBy", "COSTS");

				aSorters.push(
					new Sorter("COSTS", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "MARK");
			}
